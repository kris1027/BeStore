...
- **AC-15**: Production has a Vercel Firewall rate limit rule on `POST` requests to `/admin/sign-in`, `/admin/mfa` and `/admin/forgot-password`: 10 per minute per IP, then deny for 10 minutes. The runbook records it.
