...
- Update from spec [0005](../0005-core-buy-loop/index.md) (AC-19, AC-20), because Cache Components streams a static shell before a page can 404: the proxy now also reads the `admin_users` row and runs `decideAdminAccess()`, answering non admins with a real 404 and `aal1` sessions with a real redirect to `/admin/mfa` (it can only deny earlier, never grant; `requireAdmin()` stays the check), and sign out is a plain form POST to the `/admin/sign-out` route handler (Origin checked, 303, full page load) instead of the `signOut` server action. The ACs of this spec are unchanged.
